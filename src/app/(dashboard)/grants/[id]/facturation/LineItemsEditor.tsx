...
type Row = { key: string; label: string; description: string; amount: string; hours: string };

let nextKey = 0;
function newRow(): Row {
  nextKey += 1;
  return { key: `new-${nextKey}`, label: "", description: "", amount: "", hours: "" };
}
...
export function LineItemsEditor({ grantProjectId, initialItems }: { grantProjectId: string; initialItems: BillingLineItemRow[] }) {
  const [rows, setRows] = useState<Row[]>(() =>
    initialItems.length > 0
      ? initialItems.map((it) => ({ key: it.id, label: it.label, description: it.description ?? "", amount: String(it.amount ?? 0), hours: it.hours != null ? String(it.hours) : "" }))
      : [newRow()]
  );
  ...
  return (
    <div className="space-y-2">
      <div className="space-y-2">
        {rows.map((r) => (
          <div key={r.key} className="grid grid-cols-1 gap-2 rounded-md border border-neutral-200 p-2 sm:grid-cols-12 sm:items-start">
            <input placeholder="Activité / poste (ex. Formation employeur — Sitegrow)" value={r.label} onChange={(e) => update(r.key, { label: e.target.value })} className={`${input} sm:col-span-5`} />
            <textarea placeholder="Détail (modules, heures, taux…)" value={r.description} onChange={(e) => update(r.key, { description: e.target.value })} className={`${input} sm:col-span-4`} rows={1} />
